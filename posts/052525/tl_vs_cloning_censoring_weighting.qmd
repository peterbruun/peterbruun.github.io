...
author: 
  - name: Peter Bruun-Rasmussen (PBRZ)
    affiliations: Targeted Learning & Causal Inference
title-block-banner: "#001965"
format:
  html:
    toc: true
    toc-location: left
    self-contained: true
---
*This report is not to be shared without agreement with the author.*  

Date: 2024-12-05  

**Collaborators:** Volker Schnecke (VLSN), Britt Wang Jensen (BTWJ), 
Alex Mourer (OAXM), and Nils Skajaa (NXCS)
...
Using targeted learning further gives the benefits of double robustness and
the possibility to use data-adaptive methods (machine learning), thus minimizing
the potential for model misspecification bias. These properties are not given 
...
Here, targeted learning is utilized through the lmtp package [1], and the 
in-house polle package build by Andreas Nordland (OANO)[2]. 

An advantage of the polle-package compared to other available packages 
(lmtp/tmle) is that it is also developed to answer questions of subgroup effects
 and to identify optimal treatment policies.
 
[1] https://www.tandfonline.com/doi/full/10.1080/01621459.2021.1955691  
[2] https://arxiv.org/abs/2212.02335 


```{r, output = FALSE}
set.seed(8)

# Development version of lmtp
if (!require("lmtp")) devtools::install_github("nt-williams/lmtp@devel")

# Development version of polle
if (!require("polle")) devtools::install_github("AndreasNordland/polle")
...
outcomes using observational data", Miguel Hernan demonstrates how 
cloning-censoring weighting can be used to estimate the true effect of 
treatment duration on survival outcomes.[1] In his example, 1 or 2 years of 
Aspirin treatment has no effect on all-cause mortality. 
 
Miguel demonstrates that a naive analysis would show an effect even though
there is non, while cloning-censoring weighting will avoid this bias. 
In the example there is no confounding or censoring. 
...
You can compare the dataset below with the dataset in the paper to see that
they are the same.  

[1] https://www.bmj.com/content/360/bmj.k182.long  
...
9, 10, ands 11 survived to the second year and are on aspirin at start of 
second year.
...
# Plot
# ggplot(results, aes(x = as.factor(t), y = estimate, color = trt)) + 
...
```{r}
res <- lmtp_contrast(treat_1yr, ref = no_treat, type = "rr")$vals
...
```{r}
res <- lmtp_contrast(treat_2yr, ref = no_treat, type = "rr")$vals
kable(res, digits=3)
```
...
```{r}
res <- lmtp_contrast(treat_2yr, ref = treat_1yr, type = "rr")$vals
kable(res, digits=3)
```
...
## Influence functions using the lava-package developed by Klaus Holst (KKZH)
est <- estimate(pe)
```
